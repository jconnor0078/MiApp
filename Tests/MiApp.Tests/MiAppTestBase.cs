...
namespace MiApp.Tests
{
    public abstract class MiAppTestBase : AbpIntegratedTestBase<MiAppTestModule>
    {
        ...
        protected void UsingDbContext(Action<MiAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MiAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MiAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MiAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MiAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MiAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MiAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MiAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MiAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MiAppDbContext, Task<T>> func)
        {
            T result;
